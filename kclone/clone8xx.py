...
# 25.4mm * 1.1 = 27.94 mm
# 25.4mm * 0.5 = 12.70 mm
distance = Decimal('25.4')

for i in range(len(data)):
    if re.findall("fp_text reference [A-Z]80", data[i]):
        ...
        for j in range(len(data)):
            if "fp_text reference "+type+"81"+number in data[j]:
            ...
            if "fp_text reference "+type+"82"+number in data[j]:
                placenew = list(place)
            ...
            if "fp_text reference "+type+"83"+number in data[j]:
                placenew = list(place)
